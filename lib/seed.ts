...
export async function seedDefaultAdmin(): Promise<void> {
  ...
  try {
    // No-op if a super-admin already exists
    const existingAdmin = db
      .prepare("SELECT id FROM user WHERE role = 'super-admin' LIMIT 1")
      .get()
    if (existingAdmin) return

    // If the target email already exists, promote it to super-admin
    const existingUser = db
      .prepare("SELECT id FROM user WHERE email = ?")
      .get(adminEmail) as { id: string } | undefined

    if (existingUser) {
      db.prepare("UPDATE user SET role = 'super-admin' WHERE id = ?").run(existingUser.id)
      console.log(`[seed] Promoted existing user "${adminEmail}" to super-admin`)
      return
    }
    ...
    db.prepare("UPDATE user SET role = 'super-admin' WHERE id = ?").run(result.user.id)
    console.log(`[seed] Default super-admin created → ${adminEmail}`)
  } catch (err) {
